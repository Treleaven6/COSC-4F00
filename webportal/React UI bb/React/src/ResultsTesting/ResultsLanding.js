...
export default class ResultsLanding extends React.Component {
  ...
  render() {
    ...
    if (this.state.ready) {
      ...
      studentList = tmpArr.map(k => (
        <li key={k}>
          <span onClick={() => this.onClickStudent(k)}>{k}</span>
        </li>
      ));
    }
    ...
    if (this.state.showSingleStudent) {
      singleStudent = <SingleStudentResult goBack={this.onBack} studentKey={this.state.studentKey} data={this.props.testResults[this.state.studentKey]} />
    }
